...
async function main() {
  ...
  for (const imagePath of largeImages) {
    ...
    console.log(`📐 ${path.basename(imagePath)}: ${info.width}x${info.height} (${(info.size / 1024).toFixed(2)} KB)`);
    
    const conversion = await convertToWebP(imagePath);
    if (conversion) {
      console.log(`✅ Converted to ${path.basename(conversion.webpPath)}: ${(conversion.webpSize / 1024).toFixed(2)} KB (${conversion.savings}% smaller)\n`);
      results.push({
        original: imagePath,
  ...
  console.log('✅ Conversion complete! Results saved to image-conversions.json');
}
